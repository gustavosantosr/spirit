...
/*GetSalidasbyDespacho end point items*/
func GetSalidasbyDespacho(t int) ([]*models.Salida, error) {
	ChequeoConnection()
	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()
	rows, err := Conexion.QueryContext(ctx, `select s.*, p.Codigo, p.Kilos, p.MetrosRecibidos, p.FechaRegistro, p.Lote, d.FechaDespacho, c.Identificacion, c.Name, u.Email, se.Name, se.Code, p.Observaciones from Salidas s
